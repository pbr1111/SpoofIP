...
int main(int argc, char *argv[]) {
	...
	if (argc != 6) {
		...
		printf("- Usage %s <source hostname/IP> <source port> <target hostname/IP> <target port> <num of packets to send>\n", argv[0]);
		exit(EXIT_FAILURE);
	}
	...
	// Calculate UDP checksum field
	calculateUDPChecksum(udp_hdr, ip_hdr, (uint8_t *)data, payload);

	for (int count = 0; count < atoi(argv[5]); count++) {
...
void calculateUDPChecksum(UDP_HDR *udp_header, IPV4_HDR *ip_header, uint8_t *data, const int data_len) {
	...
	udp_header->udph_chksum = calculateIPChecksum((uint16_t*)pseudogram, psize);
	free(pseudogram);
}